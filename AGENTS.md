...
- **Play with external app — Android only (2026-09)**: the player ⋮ sheet (Playback settings) has a `Play with external app` row. It pauses playback and fires an Android `Intent.ACTION_VIEW` chooser (`createChooser(intent, "Play with…")`, `video/*` MIME) so the user can hand the current video to any installed player (MX Player, VLC, Nova, Just Player, Kodi, Plex, …). Source URI resolution in `player_screen._playExternal`: local `path` → `file://`, SAF/CX-Explorer `content://` and plain `http(s)://` URIs pass through directly; sources that need in-app credentials (WebDAV Basic, Jellyfin token URLs, in-app SMB) can't be shared as a standalone URL and show a snackbar instead. Native `playExternal` channel method in `ExoPlayerView.kt` (builds the intent + `activity.startActivity`, maps `ActivityNotFoundException` → `no_player` error) + Dart `ExoPlayerController.playExternal`. **iOS is intentionally out of scope**: iOS sandboxing prevents one app from launching another's video player (see "Player engine choice" / backlog item 6).

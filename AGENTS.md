...
- Tests: 364 passing (`flutter analyze` clean; `widget_test.dart` timer flakes fixed via binding-name `_inTests` + trial-intro mock). Incl. no-overflow checks on small phone, tablet, landscape, 2.0x text scale, and the TMDB-details/Jellyfin server-list overflow regression states, a file-browser back-navigation test (back goes up one folder at a time; a folder inside a root lands on that root's contents, not the roots list), an About → "Open-source licenses" navigation test, Jellyfin model/URL unit tests (incl. `JellyfinItem` IndexNumber/ParentIndexNumber → `SxxExx`, `videoItem` building the playable `VideoItem`, and `JellyfinItemInfo` fromApi URL construction + cache round-trip + folder-meta store), TMDB filename-parser / metadata-store round-trip tests (incl. the 2026-08 search-query regressions: bracket/paren audio-metadata stripping, bitrate + release-group/site suffix cutting, and underscore-glued episode tags), `TmdSeason`/`TmdEpisode` JSON round-trip + "Episode N" fallback + cast/stills round-trip + `withEpisode` tests (the dual-key `fromJson` regression), library-folder store unit tests (`test/library_folders_test.dart`, incl. the Jellyfin-source round-trip + legacy `source`-less default), UPnP/DLNA model tests (`UpnpEntry` transcoded/externalSubs, `VideoItem.isTranscoded` round-trip), plus `player_error_test.dart` (Media3 `ERROR_CODE_*` → friendly message + retry/decode predicates — 9 new tests for the 2026-08-29 hardware-decoder auto-fallback work), plus 2026-09-23: `mpv_audio_select_test.dart`, `audio_track_store_test.dart`, `tone_map_store_test.dart`, `settings_tone_map_platform_test.dart`, and issue #26 season-folder / filename-season / Komi-san alias / GuP non-grouping regressions, plus issue #24 MPV picture-adjustment range coverage.
- **TheTVDB coverage (2026-09-24)**: `test/the_tvdb_client_test.dart` covers v4 auth, provider mapping, secure credential migration, pagination, default season order, duplicate seasons, and request errors; `test/metadata_provider_test.dart` covers provider-qualified cache serialization and round-trips.
...
- **Year-less folder names resolve to the wrong same-titled TMDB duplicate (2026-09, Kakegurui Twin 2021 vs 2022)**: a folder named `Kakegurui Twin-1080p BD` (year only inside the files — `kakegurui twin (2021) s01e01.mkv`) resolved to the higher-popularity 2022 Netflix listing (`KAKEGURUI TWIN`, id 138183) because both entries matched 1.00 and `bestForQuery` kept the first (popularity-sorted) result. Fix: a **`yearHint` pipeline**. `ParsedFileName.yearFromNames(Iterable<String>)` returns the most-common year parsed from the folder's file names; `LibraryFolder` gained a persisted `yearHint` field (JSON round-trip); `TmdService.resolveFolder(key, name, {yearHint})` feeds `parsed.year ?? yearHint` into `bestForQuery` → `search(first_air_date_year:)`, which returns exactly the right entry. Wired at every resolution point where the listing is in scope: SMB + WebDAV `_bookmarkCurrentFolder`, the home "Add folder to library" local picker (lists the picked folder best-effort), SMB/WebDAV/FTP/UPnP/`folder_screen` series detection, home `_resolveFolderMetadata`, `TmdDetailsScreen` folder mode, and `folder_screen._resolveMeta`. Verified live: `search/tv?query=Kakegurui Twin&first_air_date_year=2021` returns only id 121860, and a matching device-side test folder now resolves to the 2021 entry. The `[01]`/`(2021)` file-name conventions are why Kakegurui-Twin style folders keep appearing in these fixes.
...
- **iOS builds happen in GitHub Actions** (user has no Mac). Workflow: `.github/workflows/ios.yml`
  ...
  - **7-day free trial**: stored in **SharedPreferences** (clears on app delete — fresh install gets a fresh trial). Keychain was removed during pre-approval testing so testers can reinstall without hitting a stale trial. **Post-approval TODO**: re-add Keychain as the primary store (survives reinstall, anti-abuse) with SharedPreferences fallback — write to both in `_persistTrialStart`, read SharedPreferences first then Keychain for migration. On reinstall, Keychain persists the start time so the trial **resumes from where it stopped** (e.g., 3 days used → 4 days remaining), not restart from day 0. `startTrial()` must detect a persisted Keychain value and restore it instead of starting fresh.

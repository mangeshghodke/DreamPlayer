...
- **iOS builds happen in GitHub Actions** (user has no Mac). Workflow: `.github/workflows/ios.yml`
  ...
  - **7-day free trial**: stored in Keychain (`trialStartedAt`, `kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly`) so it survives restart AND reinstall on the same device. The `ios/signing_setup.py` script does NOT touch Keychain trial state — that's handled by `ios/Runner/TrialClient.swift` at runtime.

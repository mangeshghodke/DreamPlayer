...
- **Background MediaStore auto-scrape (2026-08, Nova-style)**: the home screen shows an **"All videos on device"** section listing every video the Android MediaStore knows about, auto-scraped for TMDB metadata in the background. `MediaScanner.kt` queries `MediaStore.Video.Media` via `ContentResolver` on a daemon thread, returning `LibraryVideo` models (id, path, title, duration, width, height, sizeBytes, dateAdded, mimeType, resolutionLabel) through the `dreamplayer/media_scanner` MethodChannel. `LibraryVideoStore` persists results with a `merge()` strategy (add new, remove deleted, sort by dateAdded desc) so the list stays fresh across app restarts. On home load, `_loadLibraryVideos()` fetches the native list and `_autoScrapeMetadata()` searches TMDB for up to 50 unscraped videos per session (keyed `scan:<path>` in `TmdStore`), showing poster cards as matches arrive. Tapping a card opens `TmdDetailsScreen` → player via `_openLocalVideo()`. The library refreshes on app foreground and on `MediaStore` content changes (`ContentObserver` on `Video.Media.EXTERNAL_CONTENT_URI`). **Removed (2026-08)**: the old `LibraryScanService` / `LibraryStore` / `library_scan.dart` / `library_scan_test.dart` — the in-app recursive folder walk was replaced by this native MediaStore query.
...
**Phase 4 — Nova-style details screen + MediaStore auto-scrape (DONE 2026-08)**:
...
- **Background MediaStore auto-scrape**: `MediaScanner.kt` queries `MediaStore.Video.Media` via `ContentResolver` on daemon thread, returning `LibraryVideo` models through `dreamplayer/media_scanner` MethodChannel. `LibraryVideoStore` persists with `merge()` strategy. Home screen shows "All videos on device" section with TMDB auto-scrape (50/session cap). Refreshes on app foreground and `MediaStore` content changes.
...
lib/
  ...
  services/native_media_scanner.dart  # Dart MethodChannel wrapper for MediaScanner.kt (iOS no-op)
  services/library_video_store.dart   # LibraryVideoStore persistence (merge strategy, SharedPreferences, ChangeNotifier)
  config/tmdb_api_key.dart        # default TMDB key from --dart-define=TMDB_API_KEY (never committed)
  screens/
    home_screen.dart            # Continue watching grid (adaptive columns) + Your-library folder grid + **All videos on device** section (MediaStore auto-scrape) + **+** FAB menu (Jellyfin / WebDAV / Add folder / Internal storage)
    ...
    tmd_details_screen.dart     # TMDB details: Nova-style episode header + info/subtitles/trailers cards + Play + Fix match search
...
android/app/src/main/kotlin/com/dreamplayer/app/
  ...
  MediaScanner.kt               # native MediaStore query via MethodChannel (dreamplayer/media_scanner)

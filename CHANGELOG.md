...
- **On-screen badge preferences (Settings → Player)** — the chip row that shows during playback (HDR format, audio codec, resolution, video codec) is now fully configurable. A new **"On-screen badges"** toggle in Settings → Player controls whether any chips appear, and underneath it are four checkboxes: **HDR format** (DV / HDR10 / HDR10+ / HLG / SDR), **Audio codec** (E-AC3 / DTS-HD / TrueHD / AAC / FLAC · channel count), **Resolution**, and **Video codec** (HEVC / H.264 / AV1). Unchecking a category hides those chips from the player top bar instantly (the ⓘ info sheet is unaffected). Both Media3 and MPV modes respect the same per-category toggles.
